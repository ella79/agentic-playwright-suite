...
- TC-07 builds its own throwaway account with `buildAccount()` directly, since it tests rejected
  login, not a precondition an existing account would satisfy.
- TC-08, TC-09 and TC-10 take the `uniqueAccount` fixture, which registers a throwaway account before
...
| ----- | ---------------------------------------------------------------------- | ----- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TC-07 | Signing in with wrong credentials is rejected                          | error | A visitor attempts to sign in with an email that is not registered            | An inline error is shown; the visitor stays on `/login`, not signed in                                                                                                                                         |
| TC-08 | A registered user can sign in with valid credentials                   | happy | A visitor signs in with a registered email and its correct password           | The visitor lands signed in, with Logged in as \<name\> visible                                                                                                                                                |
| TC-09 | Logging out returns the visitor to the guest state                     | happy | A signed-in visitor logs out                                                  | The visitor lands on `/login`; the header no longer offers Logout                                                                                                                                              |
| TC-10 | A signed-in user can delete their account, with the deletion confirmed | happy | A signed-in visitor deletes their account and continues past the confirmation | Account Deleted! and "Your account has been permanently deleted!" are shown; `GET /api/getUserDetailByEmail` confirms the account no longer exists; continuing lands on the home page, back in the guest state |
...
- TC-10 cross-validates the deletion against `GET /api/getUserDetailByEmail` (`AccountApiClient`),
  which answers 404 once the account is gone, rather than trusting the UI banner alone.
